...
class AssessmentRequestDoclistRepo {

  GeneralFunction generalFunction = GeneralFunction();

  Future<List<Map<String, dynamic>>?> getAssessmentDataDocs(BuildContext context, licenseRequestId) async {

    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? sToken = prefs.getString('sToken');
    String? iUserId = prefs.getString('iUserId');
    String? contactNo = prefs.getString('sContactNo');
    print("---22-licenseRequesrId--$licenseRequestId");
    print('---contact no---$contactNo');

    try {
      var baseURL = BaseRepo().baseurl;
      var endPoint = "PropertyAssessmentReqDocList/PropertyAssessmentReqDocList";
      ...
      request.body = json.encode({
        "sHouseHoldRequestId":licenseRequestId,
      });
